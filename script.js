...
function getViewSize() {
  // Prefer visualViewport so we match the actual visible area (avoids URL bar overlap)
...
function resizeRenderer() {
  ...
  // Keep canvas CSS size in sync with visual viewport
...
/* ============ Menu wiring ============ */
document.getElementById("btn-new").onclick = async () => { await game.startNewGame(); game.input.requestLock(); tryFullscreen(); };
document.getElementById("btn-continue").onclick = async () => { await game.continueGame(); game.input.requestLock(); tryFullscreen(); };
...
// Fullscreen + hide browser chrome on mobile when possible
function tryFullscreen() {
  ...
  if (req) {
    try { req.call(el, { navigationUI: "hide" }); } catch (_) {
      try { req.call(el); } catch (__) {}
    }
  }
  // Nudge layout after chrome changes
...
if (isTouch) {
  ...
  // Enter fullscreen on first interaction to hide URL bar
  let fsTried = false;
  const onFirstTouch = () => {
    if (fsTried) return;
    fsTried = true;
  ...
  renderer.domElement.addEventListener("touchstart", (e) => {
    ...
    const t = e.changedTouches[0];
    // Adaptive joystick deadzone (smaller on landscape)
    const joySize = window.innerHeight <= 500 ? 130 : 170;
    if (t.clientX < joySize && t.clientY > window.innerHeight - joySize) return;
